...
;(function (factory) {
	...
	Plugin.prototype = {
		...
		unbind: function unbind() {
			...
			this.element.off('.' + pluginName, '[' + this.keys['data'] + ']').off('.' + pluginName, '[' + this.keys['data-element'] + ']').off('.' + pluginName, '[' + this.keys['event'] + ']');
		...
		_setEventListeners: function _setEventListeners() {
			...
			this.element.on(bindEvents.join(' '), '[' + this.keys['data'] + ']', function (event) {
				...
				if (typeof context.callbacks.set === 'function') {
					context.callbacks.set(element, targetKey, value, { type: event.type });
				} else if (typeof context.callbacks.main === 'function') {
					context.callbacks.main('set', element, targetKey, value, { type: event.type });
				}
			});
			...
			this.element.on(onEvents.join(' '), '[' + this.keys['event'] + ']', function (event) {
				...
				context._extractActions(element, event, actionData, function (eventType, handlerName) {
					...
					if (typeof context.callbacks.on === 'function') {
						context.callbacks.on(element, name, value, { type: event.type, args: args });
					} else if (typeof context.callbacks.main === 'function') {
						context.callbacks.main('on', element, name, value, { type: event.type, args: args });
					}
		...
		_readElementValue: function _readElementValue(element, oldValue) {
			...
			var customValue = $(element).attr(this.keys['event-value']);
